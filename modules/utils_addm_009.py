...
#        break
    ...
    for x in value_pairs:
        ...
        reject = data[data.resp == -1]             		# select reject responses
        accept = data[data.resp == 1]              		# select accept responses

        if dist_type == "kde":                          # for sim distributions

            if reject.shape > 4:
                reject_dist = gaussian_kde(
                    reject, ((reject.size * (1 + 2) / 4.)**(-1. / (1 + 4))) / 2)
            else:
                reject_dist = 0
            if accept.shape > 4:
                accept_dist = gaussian_kde(
                    accept, ((accept.size * (1 + 2) / 4.)**(-1. / (1 + 4))) / 2)
            else:
                accept_dist = 0

            rtDist[extracted_parameters][vp][reject] = reject_dist
            rtDist[extracted_parameters][vp][accept] = accept_dist

        elif dist_type == "count":                          # for sim subjects
            # Create RT distrib (counts/bin)
            count0, bins = np.histogram(reject.rt, bins)  	# unpack the reject counts in each bin
            count1, bins = np.histogram(accept.rt, bins)  	# unpack the accept counts in each bin
